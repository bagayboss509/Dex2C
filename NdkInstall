...
#fi
################################################# code ajou emba
# Vérification si le NDK est déjà installé
if [ -d "$ndk_dir" ]; then
    echo "Le NDK version $ndk_ver_name est déjà installé."
    # Si vous ne voulez pas réinstaller, vous pouvez arrêter le script ici
    echo "Aucune réinstallation nécessaire. Fin du script."
    exit 0
fi
